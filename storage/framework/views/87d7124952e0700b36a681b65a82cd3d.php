    ...
    <div class="flex justify-between items-center mb-8">
        ...
        <div class="flex gap-4">
            <div class="bg-white/80 rounded-xl p-4 shadow-md border border-[#72383D]/10">
                ...
                <p class="text-2xl font-bold text-[#401B1B]">₱<?php echo e(number_format($sales->sum('total_amount'), 2)); ?></p>
            </div>
            <div class="bg-white/80 rounded-xl p-4 shadow-md border border-[#72383D]/10">
                <p class="text-sm text-[#72383D]">Total Interest</p>
                <p class="text-2xl font-bold text-[#401B1B]">₱<?php echo e(number_format($sales->sum('interest_earned'), 2)); ?></p>
            </div>
        </div>
    </div>
                    ...
                    <tbody class="divide-y divide-[#72383D]/10">
                        <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $sales; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sale): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="hover:bg-[#F2F2EB]/50 transition-colors duration-200">
                                <td class="px-6 py-4 text-[#401B1B] font-medium">#<?php echo e($sale->id); ?></td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 text-xs font-medium rounded-full 
                                        <?php echo e($sale->type === 'loan' ? 'bg-[#AB644B]' : 'bg-[#72383D]'); ?> text-white">
                                        <?php echo e(ucfirst($sale->type)); ?>

                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-[#401B1B] font-medium"><?php echo e($sale->customer->name); ?></div>
                                    <div class="text-[#72383D] text-sm">ID: #<?php echo e($sale->customer->id); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-[#401B1B] font-medium">₱<?php echo e(number_format($sale->total_amount, 2)); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-[#72383D]">₱<?php echo e(number_format($sale->interest_earned, 2)); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="text-[#401B1B]"><?php echo e($sale->completion_date->setTimezone('Asia/Manila')->format('M d, Y')); ?></div>
                                    <div class="text-[#72383D] text-sm"><?php echo e($sale->completion_date->setTimezone('Asia/Manila')->diffForHumans()); ?></div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="px-3 py-1 text-xs font-medium rounded-full 
