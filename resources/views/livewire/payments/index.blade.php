<div class="bg-gradient-to-br from-[#F2F2EB] to-[#D2DCE6] min-h-screen p-6">
        ...
        <div class="bg-white/80 rounded-xl shadow-lg p-6 border border-[#72383D]/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#401B1B]">₱{{ number_format($totalPayments ?? 0, 2) }}</h3>
                </div>
        ...
        <div class="bg-white/80 rounded-xl shadow-lg p-6 border border-[#72383D]/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#401B1B]">{{ $pendingCount ?? 0 }}</h3>
                </div>
        ...
        <div class="bg-white/80 rounded-xl shadow-lg p-6 border border-[#72383D]/10">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <h3 class="text-2xl font-bold text-[#401B1B]">₱{{ number_format($todayPayments ?? 0, 2) }}</h3>
                </div>
                <div class="bg-[#72383D]/10 p-3 rounded-full">
                    <svg class="w-6 h-6 text-[#72383D]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                @forelse($paymentSubmissions as $submission)
                    <div class="bg-[#F2F2EB]/50 p-4 rounded-lg border border-[#72383D]/10">
                        <div class="flex justify-between items-start">
                            <div>
                                <p class="font-medium text-[#401B1B]">{{ $submission->customer->name }}</p>
                                <p class="text-sm text-[#72383D]">Amount: ₱{{ number_format($submission->amount, 2) }}</p>
                                <p class="text-xs text-[#72383D]">Submitted: {{ $submission->created_at->diffForHumans() }}</p>
                            </div>
                            <button 
                ...
                <tbody class="divide-y divide-[#72383D]/10">
                    @forelse($this->payments as $payment)
                        <tr class="hover:bg-[#F2F2EB]/50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-[#401B1B]">{{ $payment->payment_date->format('M d, Y') }}</div>
                                <div class="text-xs text-[#72383D]">
                            ...
                            <td class="px-6 py-4">
                                @if($payment->status === 'active')
                                    <button 
                                        wire:click="confirmVoid({{ $payment->id }})"
            ...
            <div class="relative bg-gradient-to-br from-[#F2F2EB] to-[#D2DCE6] rounded-lg max-w-4xl w-full border-2 border-[#72383D]/20 shadow-xl">
                <div class="p-6">
                    ...
                    @if($selectedCustomer)
                        ...
                        @if($selectedAR && $selectedCustomerPayments->isNotEmpty())
    ...
    <div x-data="{ open: @entangle('reversalModalOpen') }" 
         x-show="open" 
         x-cloak
         class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-gradient-to-br from-[#F2F2EB] to-[#D2DCE6] rounded-lg max-w-lg w-full border-2 border-[#72383D]/20 shadow-xl">
                <div class="p-6">
                    ...
                    @if($reversalStage === 'confirm')
                        <div class="bg-white/50 p-4 rounded-lg shadow-inner mb-4">
                            <p class="text-[#401B1B]">
                                Are you sure you want to reverse the payment of ₱{{ number_format($paymentToReverse?->amount_paid ?? 0, 2) }}?
                            </p>
                        </div>
                        <div class="flex space-x-2">
                            <x-button 
                                wire:click="confirmReversal"
                                class="bg-[#72383D] hover:bg-[#401B1B] text-white"
                                label="Yes, Reverse"
                            />
                            <x-button 
                                wire:click="cancelReversal"
                                class="bg-[#AB644B] hover:bg-[#72383D] text-white"
                                label="Cancel"
                            />
                        </div>
                    @elseif($reversalStage === 'edit')
                        <div class="space-y-4">
                            ...
                            <div class="bg-white/50 p-4 rounded-lg shadow-inner">
                                ...
                                <x-input 
                                    :value="number_format($this->updatedRemainingBalance, 2)"
                                    type="text"
                                    readonly
                                    disabled
                            ...
                            <div class="flex space-x-2 mt-4">
                                <x-button 
                                    wire:click="processReversalWithNewPayment"
                                    class="bg-[#72383D] hover:bg-[#401B1B] text-white"
                                    label="Save Changes"
                                />
                                <x-button 
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white/50 p-4 rounded-lg">
                            <p class="text-[#72383D]">Amount: ₱{{ number_format($viewingSubmission->amount ?? 0, 2) }}</p>
                            <p class="text-[#72383D]">Customer: {{ $viewingSubmission->customer->name ?? 'N/A' }}</p>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white/50 p-4 rounded-lg">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <p class="font-medium text-[#401B1B]">{{ $viewingSubmission->customer->name }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Amount</p>
                                    <p class="font-medium text-[#401B1B]">₱{{ number_format($viewingSubmission->amount, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Due Amount</p>
                                    <p class="font-medium text-[#401B1B]">₱{{ number_format($viewingSubmission->due_amount, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Payment Date</p>
                                    <p class="font-medium text-[#401B1B]">{{ $viewingSubmission->payment_date->format('M d, Y') }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-white/50 p-4 rounded-lg">
                            ...
                            <img 
                                src="{{ Storage::url($viewingSubmission->payment_proof) }}" 
                                alt="Payment Proof"
                        ...
                        <div class="flex justify-end space-x-3">
                            <x-button 
                                wire:click="$set('submissionModalOpen', false)"
                                class="bg-[#9CABB4] hover:bg-[#72383D] text-white"
                                label="Close"
                            />
                            <x-button 
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-white/50 p-4 rounded-lg">
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    <p class="text-sm text-[#72383D]">Customer</p>
                                    <p class="font-medium text-[#401B1B]">{{ $selectedPayment->customer->name }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Amount</p>
                                    <p class="font-medium text-[#401B1B]">₱{{ number_format($selectedPayment->amount_paid, 2) }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Payment Date</p>
                                    <p class="font-medium text-[#401B1B]">{{ $selectedPayment->payment_date->format('M d, Y') }}</p>
                                </div>
                                <div>
                                    <p class="text-sm text-[#72383D]">Status</p>
                                    <p class="font-medium text-[#401B1B] capitalize">{{ $selectedPayment->status }}</p>
                                </div>
                            </div>
                        </div>
