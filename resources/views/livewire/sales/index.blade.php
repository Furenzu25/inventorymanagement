                ...
                <tbody class="divide-y divide-[#72383D]/10">
                    @foreach($sales as $sale)
                        <tr class="hover:bg-[#F2F2EB]/50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm font-medium text-[#401B1B]">{{ $sale->customer->name }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-[#401B1B]">
                                    @if($sale->accountReceivable && $sale->accountReceivable->preorder)
                                        @foreach($sale->accountReceivable->preorder->preorderItems as $item)
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-[#401B1B]">₱{{ number_format($sale->total_amount, 2) }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-[#401B1B]">₱{{ number_format($sale->interest_earned, 2) }}</div>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full
                                    {{ $sale->type === 'customer_payment' ? 'bg-[#72383D]/10 text-[#72383D]' : 'bg-[#AB644B]/10 text-[#AB644B]' }}">
                                    {{ str_replace('_', ' ', ucfirst($sale->type)) }}
                                </span>
                            </td>
                            <td class="px-6 py-4">
                                <span class="px-2 py-1 text-xs font-semibold rounded-full
